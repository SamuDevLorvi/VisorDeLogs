...
const showRow = async (click, row) => {
  detalles.value = row.item
  console.log(row.item)
  await router.push({name:'detalle', params: {index: row.index}})
  console.log("parametros de ruta: " + route.fullPath)
}
</script>

<template>
  <h1 style="text-align: center; background-color: #5c6bc0">Tabla</h1>
  <v-data-table
    :headers="headers"
    :items="items"
    item-key="name"
